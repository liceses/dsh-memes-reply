...
import type { MemesConfig, StickerBorderStyle, StickerShape } from './types.js'
...
/** 边框颜色：空串 = 跟随主题强调色。 */
function borderColorOf(config: MemesConfig): string {
...
function borderStyleOf(style: StickerBorderStyle, width: number): string {
...
export function themeCss(config: MemesConfig): string {
  const shape: StickerShape = config.shape === 'rounded' ? 'rounded' : 'circle'
