...
export function modelPickOf(argumentsRaw: unknown): { id: string | null; mood: string | null } {
 ...
 */
export function closingTextOf(closing: ClosingAssistantLike | null | undefined): string {
  const blocks = closing?.blocks
  if (blocks === undefined) return ''
  const parts: string[] = []
  for (const block of blocks) {
    if (block.kind === 'text' && typeof (block as { text?: unknown }).text === 'string') {
 ...
 */
export function closingPickOf(closing: ClosingAssistantLike | null | undefined): {
  id: string | null
  mood: string | null
} {
  const blocks = closing?.blocks
  if (blocks === undefined) return { id: null, mood: null }
  let found: { id: string | null; mood: string | null } | null = null
  for (const block of blocks) {
    if (block.kind !== 'tool-call') continue
    const call = block as { name?: unknown; argsRaw?: unknown }
