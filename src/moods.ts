...
/** 搜索用的索引（一次性建好）。 */
const BY_KEY = new Map<string, MoodFamily>(MOOD_FAMILIES.map((family) => [family.key, family]))

/** 按 key 取一族。 */
export function familyOf(key: string | null | undefined): MoodFamily | undefined {
  if (key === null || key === undefined || key === '') return undefined
  return BY_KEY.get(key)
}

/** JEV choice 的 criteria：`{familyKey: gloss}` + blank。 */
export function familyCriteria(): Record<string, string> {
  const criteria: Record<string, string> = {}
  for (const family of MOOD_FAMILIES) criteria[family.key] = family.gloss
 ...
 */
export function allFamilyMembers(): string[] {
  const out: string[] = []
  for (const family of MOOD_FAMILIES) out.push(...family.members)
  return out
}
