 ...
 */
export function pickByKeyword(entries, text, avoid) {
    const haystack = text.toLowerCase();
    if (haystack.trim() === '')
        return null;
    let best = null;
    for (const entry of entries) {
        if (avoid.has(entry.id))
            continue;
        let bestTerm = '';
        for (const raw of [entry.name, ...entry.tags, ...entry.aliases]) {
            const term = raw.trim().toLowerCase();
            if (term.length < AUTO_KEYWORD_MIN_TERM_LEN)
                continue;
        ...
        if (best === null || score > best.score)
            best = { entry, matched: bestTerm, score };
    }
...
/** 「每 N 轮」模式：确定性挑一张没用过的。 */
export function pickEvery(entries, sessionId, turn, avoid) {
...
export function pickAutoSticker(input) {
    ...
    if (input.mode === 'keyword')
        return pickByKeyword(input.entries, input.text, input.avoid);
