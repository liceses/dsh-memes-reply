...
import { effectiveAssetRoot, loadIndex, resolveStickerFile, resolveThumbFile } from './assets.js';
import { CATALOG_DEFAULT_LIMIT, CATALOG_MAX_LIMIT } from './config.js';
import { createDecisionCache, createJevLog, decideJev, pickInFamily, } from './jev.js';
import { CATALOG_PATH, DEBUG_PATH, JEV_LOG_PATH, JEV_PATH, LATCH_PATH, MAX_STICKER_BYTES, MIME, PET_PATH, ROUTE_PREFIX, SESSION_STATE_PATH, STATS_PATH, STICKER_ID_RE, STICKER_PATH, THUMB_PATH, VOCAB_PATH, extensionOf, stickerUrl, thumbUrl, } from './protocol.js';
...
function clampInt(raw, fallback, min, max) {
    const clamp = (value) => Math.min(max, Math.max(min, Math.trunc(value)));
...
async function sendBytes(req, res, resolved, etag, file, maxBytes, onServed, ctx) {
    const headers = {
...
/** 构建这一条前缀路由。 */
export function createStickerRoute(deps) {
    ...
    return {
        ...
        handler: async (req, res) => {
            ...
            if (observed !== '')
                deps.observeOrigin(`http://${observed}`);
            let url;
            try {
            ...
            if (pathname === JEV_PATH || pathname === `${JEV_PATH}/`) {
                ...
                const decision = await decideJev({
                    ...
                    readFile: deps.jev?.readFile ?? ((path) => readFileSync(path, 'utf8')),
                    env: deps.jev?.env ?? process.env,
                    credentialsPath: deps.jev?.credentialsPath ?? credentialsPath(),
                    now: deps.jev?.now ?? (() => Date.now()),
                }, { reply: text, userText, recent, persona: cfg.jevPersona }, { model: cfg.jevModel, timeoutMs: cfg.jevTimeoutMs });
                ...
                stats.jev.calls += 1;
                if (decision.costUsd !== null)
                    stats.jev.costUsd += decision.costUsd;
                stats.jev.lastMs = decision.ms;
                stats.jev.lastNote = decision.note;
                /** 族内挑一张：JEV 给了情绪族，"具体哪张"由代码定（去重 + 确定性）。 */
                const picked = decision.stick
            ...
            if (pathname === PET_PATH || pathname === `${PET_PATH}/`) {
                ...
                if (input.slot === 'jevDebug') {
                    ...
                    if (coords.right !== undefined)
                        panelPatch.right = coords.right;
                    if (coords.bottom !== undefined)
                        panelPatch.bottom = coords.bottom;
                    if (typeof input.collapsed === 'boolean')
                        panelPatch.collapsed = input.collapsed;
                ...
                const patch = {};
                if (coords.right !== undefined)
                    patch.right = coords.right;
                if (coords.bottom !== undefined)
                    patch.bottom = coords.bottom;
                if (typeof input.collapsed === 'boolean')
            ...
            const etag = resolved.quality === 'compressed' && entry.sha256 !== ''
                ? `"${entry.sha256.slice(0, 32)}"`
                : `"o${resolved.info.size ?? 0}-${String(resolved.info.version).slice(0, 16)}"`;
            await sendBytes(req, res, resolved, etag, entry.file, MAX_STICKER_BYTES, () => {
                stats.served += 1;
                stats.lastId = id;
