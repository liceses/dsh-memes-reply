window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const UNAVAILABLE_SNAPSHOT$1 = Object.freeze({ status: "unavailable" });
		...
		*/
		function closingTextOf(closing) {
			const blocks = closing?.blocks;
			if (blocks === void 0) return "";
			const parts = [];
			for (const block of blocks) if (block.kind === "text" && typeof block.text === "string") parts.push(block.text);
			return parts.join("\n");
		}
		/**
		...
		*/
		function closingPickOf(closing) {
			const blocks = closing?.blocks;
			if (blocks === void 0) return {
				id: null,
				mood: null
			...
			for (const block of blocks) {
				...
				const call = block;
				if (call.name !== "use_sticker") continue;
		...
		*/
		function StickerNodeView({ scope, seat = "flow", ...props }) {
		...
		*/
		function installStickerNode(ctx, scope, insideSession) {
...
`;
		...
		* "没有内容的条目渲染成 null"。
		*/
		function StickerTurnTail({ scope, ...props }) {
			const fallbackTurn = typeof props.turn?.turn === "number" ? props.turn.turn : 0;
			const source = props.turn?.data?.source;
			const tailSource = typeof source === "function" ? source(TURN_TAIL_KEY) : void 0;
			const subscribe = (0, react.useCallback)((listener) => tailSource === void 0 ? () => {} : tailSource.subscribe(listener), [tailSource]);
			const read = (0, react.useCallback)(() => tailSource === void 0 ? void 0 : tailSource.get(), [tailSource]);
			const tail = (0, react.useSyncExternalStore)(subscribe, read, read);
			...
			(0, react.useEffect)(() => {
				postDebug({
					...
					note: `seat=turn-tail turn=${tail?.turn ?? fallbackTurn} seq=${tail?.seq ?? props.seq ?? "?"} tail=${tail === void 0 ? "undefined" : "ok"} closing=${closing === null ? "null" : "ok"} text=${text.length}字 pick=${pick.id ?? pick.mood ?? "无"}`
				});
			}, [
				tail?.turn,
				tail?.seq,
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(StickerNodeView, {
				scope,
				seat: "turn-tail",
		...
		*/
		function installStickerTurnTail(ctx, scope) {
			const anyCtx = ctx;
			anyCtx.slots.inject(TURN_TAIL_SLOT, () => {
				postDebug({
					kind: "sticker-turn-tail-slot",
					note: `槽位已声明 → 注册 ${ENTRY_ID}`
				});
				return anyCtx.slots.register({
					name: TURN_TAIL_SLOT,
					id: ENTRY_ID,
					order: ENTRY_ORDER
				}, (props) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(StickerTurnTail, {
					scope,
					...props
				}));
			});
			postDebug({
				...
				note: `slot=${TURN_TAIL_SLOT} id=${ENTRY_ID}（落定贴纸的新座位；旧的自定义节点只留生成中占位） — 若迟迟没有 sticker-turn-tail-slot 回执，说明槽位没被声明（或注册不在会话 fiber 里）`
			});
		}
		//#endregion
		//#region src/client/index.tsx
		/** 客户端构建标记：每次改客户端就换一个，刷新后从 `/stats` 的 `client-apply` 回执里核对。 */
		const CLIENT_BUILD = "turn-tail-b";
		/**
		* 顶层硬依赖。
		*
		...
		*/
		...
		function apply(ctx) {
			...
			ctx.inject(["settingsScope"], (inner) => {
				const service = inner.settingsScope;
				if (service === void 0 || typeof service.bind !== "function") return;
				const live = service.bind({ namespace: SETTINGS_NS });
				inner.effect(() => scope.attach(live), "dsh-memes-reply: settings attach");
				postDebug({
					kind: "client-settings",
					note: `build=${CLIENT_BUILD} settings=attached`
				});
			});
			ctx.effect(() => {
