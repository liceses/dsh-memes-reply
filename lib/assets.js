...
/** 包内兜底素材目录（`lib/../assets`）。 */
export const PACKAGED_ASSET_ROOT = join(dirname(fileURLToPath(import.meta.url)), '..', 'assets');
const cache = new Map();
/** 生效的压缩副本目录。 */
export function effectiveAssetRoot(cfg) {
    return cfg.assetRoot !== '' ? cfg.assetRoot : defaultAssetRoot();
}
/** 索引候选路径（按优先级）。 */
function indexCandidates(cfg) {
 ...
 */
export async function resolveStickerFile(ctx, entry, cfg) {
    const fs = ctx.get('fs');
    if (fs === undefined)
        return undefined;
    const candidates = [];
 ...
 */
export async function resolveThumbFile(ctx, entry, cfg) {
    const fs = ctx.get('fs');
    if (fs === undefined)
        return undefined;
    const file = thumbFileOf(entry);
    const roots = [join(effectiveAssetRoot(cfg), 'thumb')];
    if (effectiveAssetRoot(cfg) !== PACKAGED_ASSET_ROOT)
        roots.push(join(PACKAGED_ASSET_ROOT, 'thumb'));
